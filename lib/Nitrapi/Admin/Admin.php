...
class Admin extends NitrapiObject
{
     ...
     *
     * @param string|int $userId The user to generate a token for
     ...
     * @param null|int $serviceId A serviceID to pin the token to. It won't be possible to access other services with this
     * token. Passing null allows all services to be accessed.
     * @param null|int $expires_in The time in seconds the new token will be valid for. The life time of a sub token
     ...
     */
    public function getUserToken($userId, $scopes, ?int $serviceId = null, ?int $expires_in = null): AccessToken {
        $scopeString = $scopes;
        if (is_array($scopes)) {
